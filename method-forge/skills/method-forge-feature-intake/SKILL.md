...
把原始请求压成一个可路由的 `intake.md`，明确下一步是直接实现、先 research，还是进入完整 spec 流。
...
3. 产出 `suggested_path`，只能在 `direct-implement`、`research-first`、`spec-flow` 中选择。
4. 明确 `next_step`，让后续执行者不必重新判断流程入口。
